...
from utils.neo4j_ingestion import delete_model_subtree, delete_nodes_by_ids, get_model_artifact_paths, update_model_node_description
...
def sanitize_model_name(model_name: str) -> str:
    """
    The model's underlying name. Markdown record file names must be safe across
    filesystems, and the same value keys the model's Neo4j Model node, so the
    record file, the Model node and the sidebar listing all agree on one name.
    Only [a-zA-Z0-9._-] is kept, anything else (e.g. spaces) becomes an
    underscore, and names that are left too short get a "model_" prefix so the
    mapping from display name to underlying name stays deterministic.
    """
...
def _txt_entry(record: dict) -> str:
    ids = ", ".join(record.get("chunk_ids") or [])
    return (
...
def _image_entry(record: dict) -> str:
    return (
...
def _document_entry(record: dict, source_type: str) -> str:
    ...
    if images:
        lines.append("- **Images:**")
        lines.extend(f"  - `{image['file_path']}` (id: {image['image_id']})" for image in images)
...
def _table_entry(record: dict) -> str:
    lines = [
    ...
    for table in record.get("tables") or []:
        ...
        lines.append(f"  - **Table id:** {table.get('table_id', '')}")
        ...
        lines.append(f"  - **Row count:** {table.get('row_count', '')}")
        lines.append("  - **Description:**")
    return "\n".join(lines) + "\n"


def _entry_for(record: dict) -> str:
    source_type = record.get("source_type")
    if source_type == "txt":
        return _txt_entry(record)
    if source_type == "image":
        return _image_entry(record)
    if source_type == "table":
        return _table_entry(record)
    if source_type in ("pdf", "ppt"):
        return _document_entry(record, source_type)
...
def _ids_from(block: str) -> list:
    ids_line = re.search(r"\*\*Chunk ids:\*\* (.*)", block)
    if not ids_line:
        return []
    return [chunk_id.strip() for chunk_id in ids_line.group(1).split(",") if chunk_id.strip()]


def remove_file_entry(model_name: str, file_name: str) -> dict:
    """
    Remove one file's entry from the model's markdown file (§6.2).

    The entry's Neo4j nodes are deleted in the same operation — its chunks,
    images, tables and the File node itself, scoped to the ids recorded in the
    entry, which is what keeps a re-uploaded same-named file's nodes safe —
    and so are the file's permanent disk artifacts (image files under
    data/images/, table csvs under data/tables/), which unlike txt temp files
    are kept on purpose. Entries created before the Neo4j switch record no
    node ids, so nothing is deleted in Neo4j for them. Only the most recent
    entry for the file name is removed, since re-uploads create separate
    entries (§4). Returns a summary dict.
    """
    summary = {"entry_removed": False, "nodes_deleted": 0, "files_removed": 0}
    content = _read_description(model_name)
    if content is None:
        return summary
    ...
    if entry_type == "image":
        image_id = re.search(r"\*\*Image id:\*\* (\S+)", block)
        if image_id:
            summary["nodes_deleted"] += delete_nodes_by_ids("image", [image_id.group(1)])
        ...
        summary["nodes_deleted"] += delete_nodes_by_ids("chunk", _ids_from(block))
        # The entry's images are Image nodes under the same File, plus files on disk.
        image_ids = re.findall(r"\(id: (\S+)\)", block)
        image_paths = re.findall(r"- `([^`]+)` \(id:", block)
        if image_ids:
            summary["nodes_deleted"] += delete_nodes_by_ids("image", image_ids)
        summary["files_removed"] += _remove_files(image_paths)

    elif entry_type == "table":
        table_ids = re.findall(r"\*\*Table id:\*\* (\S+)", block)
        if table_ids:
            summary["nodes_deleted"] += delete_nodes_by_ids("table", table_ids)
    ...
    file_id = re.search(r"\*\*File id:\*\* (\S+)", block)
    if file_id:
        summary["nodes_deleted"] += delete_nodes_by_ids("file", [file_id.group(1)])
...
def delete_model_record(model_name: str) -> bool:
    """Delete the model's markdown file (whole-model deletion, §6.2). The Neo4j subtree is removed by delete_model."""
    path = description_path(model_name)
...
def recorded_models() -> list[str]:
    ...
    file — including models with no Model node in Neo4j at all (e.g. records
    created before the switch). The sidebar unions these with the Model node
    names so such models stay reachable and deletable.
    """
...
def update_model_display_name(model_name: str, new_name: str) -> str:
    ...
    specs/data_model_editing.md — display-name only: the Model node and the
    markdown file name stay untouched). Raises ValueError when the new name
...
def update_model_description(model_name: str, new_description: str) -> bool:
    """
    Rewrite the header's Description line (§2) and, in the same operation, the
    Model node's description. An empty value is a no-op returning False.
    """
    ...
    _write_description(model_name, _touch(content))
    update_model_node_description(sanitize_model_name(model_name), new_description)
    return True
...
def delete_model(model_name: str) -> dict:
    """
    Delete the whole model (§6 of specs/data_model_editing.md): the Model node
    and its entire subtree in Neo4j, the disk files it references (only under
    data/images/ and data/tables/), and the markdown record. Each step is
    attempted independently — a missing node or file is not an error. Returns
    a summary dict of what was removed, with an "errors" list naming anything
    that existed but could not be deleted (so failures surface in the UI
    instead of being silently skipped).
    """
    summary = {"nodes_deleted": 0, "files_removed": 0, "description_removed": False, "errors": []}

    # Disk files to remove: every data/images/ and data/tables/ path mentioned
    # in the model's record, plus the ones the model's Neo4j subtree points at
    # (the record paths are the fallback that keeps file cleanup working even
    ...
    try:
        referenced_paths.extend(get_model_artifact_paths(model_name))
    except Exception as error:
        summary["errors"].append(f"could not read the model's Neo4j nodes: {error}")

    try:
        summary["nodes_deleted"] = delete_model_subtree(model_name)
    except Exception as error:
        summary["errors"].append(f"could not delete the model's Neo4j nodes: {error}")
